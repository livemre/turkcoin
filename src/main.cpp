...
#if defined(NDEBUG)
# error "Turkcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xaf4bb3bf49ed71a0c3906aa04126c8353d1fbdba34948813b6fabd9d7044c9cc");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Turkcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Turkcoin Signed Message:\n";
...
{
    // Turkcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Turkcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Turkcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Turkcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Turkcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Turkcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xed4cedaef3549d3f357303b794dbffb805aa1437d35a26d142048fe8e71af8fc");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Türkiye Türklerindir.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1516876525;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085440355;

        if (fTestNet)
        {
            block.nTime    = 1516876480;
            block.nNonce   = 385294748;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xa8bbbc76146b7ecdb3aa58342636b99925b501f5177929b8b701ca269ad090d3"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc4, 0xb3, 0xda }; // Turkcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TurkcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TurkcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TurkcoinMiner(CWallet *pwallet)
{
    printf("TurkcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("turkcoin-miner");
    ...
    try { loop {
        ...
        printf("Running TurkcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TurkcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TurkcoinMiner, pwallet));
}
